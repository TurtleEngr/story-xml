...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--
$Header: /repo/local.cvs/app/story-xml/src/story2/story-toc.xsl,v 1.3 2008/01/07 02:35:52 bruce Exp $
-->
  <xsl:output method="xml"
	      indent="yes" />
  <xsl:param name="toc-depth"
	     select="'5'" />
  <xsl:param name="title-limit"
	     select="'50'" />
  <xsl:template match="/*">
    <xsl:choose>
      <xsl:when test="node()">
  ...
  <xsl:template match="*"
	...
	<xsl:choose>
	  ...
	  <xsl:otherwise>
		...
		<xsl:apply-templates select="*[node()[1]/self::text()][1]"
				     xse:sections="omit" />
	      </xsl:when>
	      <!-- Process only first child that have text children, and all
		   those who have no text children -->
	      <xsl:otherwise>
		<xsl:apply-templates select="*[node()[1]/self::text()][1]|*[not (text())]"
				     xse:sections="omit" />
	      </xsl:otherwise>
	    </xsl:choose>
	  </xsl:otherwise>
