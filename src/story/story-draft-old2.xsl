...
<xsl:stylesheet version="1.0"
  ...
  <!--
$Header: /repo/local.cvs/app/story-xml/src/story4/story-draft-old2.xsl,v 1.1 2008/02/24 21:38:26 bruce Exp $
-->
  ...
  <xsl:template name="fOutputList">
    <exsl:document method="text"
      ...
      <xsl:apply-templates select="book"
			   mode="output-list" />
      <xsl:call-template name="tTimelineOutputList" />
    </exsl:document>
  </xsl:template>
  ...
  <xsl:template name="tTimelineOutputList">
  ...
  <xsl:template match="book"
	...
	<html>
	  ...
	  <br />
	  <xsl:call-template name="fShowDraft">
...
<xsl:call-template name="fCVSPprint" />
  ...
  <xsl:template match="chapter"
    ...
    <xsl:if test="contains($gChStatus, concat(' ch-', @status, ' ')) and (($gContentCh = '') or contains($gContentCh, concat(' ', @id, ' ')))">
      ...
      <xsl:call-template name="fChapterBodyDraft" />
    </xsl:if>
  </xsl:template>
  <!-- ******************** -->
  <xsl:template name="fChapterBodyDraft">
    <hr />
    <a>
...
<xsl:call-template name="fCVSPprint" />
  ...
  <xsl:template match="unit"
	...
	<xsl:call-template name="fOutline"
			   mode="draft" />
      </xsl:if>
      <p />
