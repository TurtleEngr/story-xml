  ...
  <xsl:template match="story-info">
    <xsl:element name="story-info">
      ...
      <xsl:if test="not(def-when)">
        <def-when year="2000" month="01" day="01" hour="08" min="01" gmt="-08"/>
      </xsl:if>
      <xsl:apply-templates />
    </xsl:element>
  </xsl:template>
...
-->
    <xsl:variable name="tDate" select="."/>
<xsl:message>
...
-->
    <xsl:variable name="tStartDate" select="../start-date"/>
    <xsl:variable name="tDate" select="date:add($tStartDate, .)"/>
<xsl:message>
